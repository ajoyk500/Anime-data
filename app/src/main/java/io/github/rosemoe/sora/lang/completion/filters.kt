...
val diag = Array(maxLen) { IntArray(maxLen) } 
...
fun fuzzyScore(
    ...
    while (patternPos < patternLen) {
        ...
        while (wordPos < nextMaxWordMatchPos) {
            ...
            val leftScore =
                if (canComeLeft) table[row][column - 1] + (if (diag[row][column - 1] > 0) -5 else 0) else 0 
            val canComeLeftLeft = wordPos > minWordMatchPos + 1 && diag[row][column - 1] > 0
            val leftLeftScore =
                if (canComeLeftLeft) table[row][column - 2] + (if (diag[row][column - 2] > 0) -5 else 0) else 0 
